...
def main(config):
    ...
    if config.warm_start:
        model.load_state_dict(paddle.load(os.path.join(config.ckpt_path, 'epoch-%d.pth' % config.warm_start_epoch)))
    ...
    optimizer = optim.Momentum(parameters=parameters, learning_rate=0.1, momentum=0.9)
    ...
    param_num = sum([param.numel() for param in model.parameters() if param.trainable])
    print('Trainable params: %.2f million' % (param_num / 1e6))
    ...
    if config.train:
        ...
        # train_loader = paddle.utils.data.DataLoader(trainset, batch_size=config.train_batch_size,
        ...
        val_loader = DataLoader(valset, batch_size=config.val_batch_size,
        ...
        for epoch in range(config.warm_start_epoch, config.epochs):
            ...
            for i, data in enumerate(train_loader):
                ...
                labels = data['annotations']#.float()
                outputs = model(images)
                # outputs = outputs.view(-1, 10, 1)
                # outputs =outputs.reshape(outputs, shape=[-1, 10, 1])
                outputs = outputs.reshape([-1, 10, 1])
            ...
            print('epoch losses', {'epoch train loss': avg_loss, 'epoch val loss': avg_val_loss}, epoch + 1)
            ...
            if avg_val_loss < init_val_loss:
                ...
                paddle.save(model.state_dict(), os.path.join(config.ckpt_path, 'epoch-%d.pth' % (epoch + 1)))
                print('Done.\n')
                # reset count
                count = 0
    ...
    if config.test:
        ...
        test_loader = paddle.utils.data.DataLoader(testset, batch_size=config.test_batch_size, shuffle=False, num_workers=config.num_workers)
...
if __name__ == '__main__':
    ...
    # input parameters
    parser.add_argument('--img_path', type=str, default='/home/data/disk3/zl/code/Neural-IMage-Assessment-original/AVA_dataset/image')
    parser.add_argument('--train_csv_file', type=str, default='/home/data/disk3/zl/code/Neural-IMage-Assessment-original/AVA_dataset/train_labels.csv')
    parser.add_argument('--val_csv_file', type=str, default='/home/data/disk3/zl/code/Neural-IMage-Assessment-original/AVA_dataset/val_labels.csv')
    parser.add_argument('--test_csv_file', type=str, default='/home/data/disk3/zl/code/Neural-IMage-Assessment-original/AVA_dataset/test_labels.csv')
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='/home/data/disk3/zl/code/SR-paddle/NIMA/ckpts')
